...
public final class NetworkManager {
	...
	public static NetworkingError createSimpleServer(final String identifier, final int listenPort) {
        if (serversMap.containsKey(identifier)) return NetworkingError.IDENTIFIER_ALREADY_TAKEN;
    ...
    /**
	 * Removes a server from the serversMap if it was previously stopped with stopServer
	 *
    ...
    /**
	 * Closes all open active connections with all clients of a server, and
     * set it to "inactive" state, allowing to safely remove it from the
     * serversMap.
	 *
    ...
    /**
	 * Returns the instance of the SimpleServer with provided identifier
     * if it exists, or null if it doesn't.
	 *
	 * @param identifier The server identifier
	 **/
    public static SimpleServer getSimpleServer(final String identifier) {
        if (!serversMap.containsKey(identifier)) return null;
